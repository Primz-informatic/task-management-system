// buka code di saat ingin testing
// const User = require('../models/User');

// PERBAIKAN: Ganti nama variabel jadi 'UserModel' agar tidak bentrok dengan class User global
const UserModel = (typeof require !== 'undefined') 
    ? require('../models/User') 
    : (window.User || User); 
...
class UserRepository {
    ...
    create(userData) {
        try {
            ...
            // PERBAIKAN: Gunakan UserModel untuk instansiasi
            const user = new UserModel(userData.username, userData.email, userData.fullName);
    ...
    _loadUsersFromStorage() {
        try {
            ...
            usersData.forEach(userData => {
                try {
                    // PERBAIKAN: Gunakan UserModel untuk akses method static fromJSON
